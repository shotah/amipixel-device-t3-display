#ifdef __has_include
#if __has_include("lvgl.h")
#ifndef LV_LVGL_H_INCLUDE_SIMPLE
#define LV_LVGL_H_INCLUDE_SIMPLE
#endif
#endif
#endif

#if defined(LV_LVGL_H_INCLUDE_SIMPLE)
#include "lvgl.h"
#else
#include "lvgl.h"
#endif

#ifndef LV_ATTRIBUTE_MEM_ALIGN
#define LV_ATTRIBUTE_MEM_ALIGN
#endif

#ifndef LV_ATTRIBUTE_IMG_GIF_GIF_CAT
#define LV_ATTRIBUTE_IMG_GIF_GIF_CAT
#endif

/*
 * Procedurally generated stick figure animation
 * Created using pure code - no external images!
 * 
 * Animation: gif_cat
 * Size: 50x50
 * Frames: 4
 */

// clang-format off
const LV_ATTRIBUTE_MEM_ALIGN LV_ATTRIBUTE_LARGE_CONST LV_ATTRIBUTE_IMG_GIF_GIF_CAT uint8_t gif_cat_map[] = {

    , 
};
// clang-format on

const lv_img_dsc_t gif_cat = {
    .header = {
        .cf = LV_COLOR_FORMAT_NATIVE,
                        .w = 50,
        .h = 50,
    },
    .data_size = sizeof(gif_cat_map),
    .data = gif_cat_map,
};

/*
 * Stick Figure Pattern (for reference):
 * 
 * Frame 1 pattern:
 * ..................................................
 * ..................................................
 * ..................................................
 * ..................................................
 * ..................................................
 * ..................................................
 * .........................#........................
 * ......................#######.....................
 * .....................#########....................
 * ....................###########...................
 * ....................###########...................
 * ....................###########...................
 * ...................#############..................
 * ....................###########...................
 * ....................###########...................
 * ....................###########...................
 * .....................#########....................
 * ......................#######.....................
 * .........................#........................
 * .........................#........................
 * .........................#........................
 * .........................#........................
 * .........................#........................
 * .........................#........................
 * ...............#####################..............
 * .........................#........................
 * .........................#........................
 * .........................#........................
 * .........................#........................
 * .........................#........................
 * .........................#........................
 * .........................#........................
 * .........................#........................
 * .........................#........................
 * .........................#........................
 * .........................#........................
 * .........................#........................
 * ........................#.#.......................
 * ........................#.#.......................
 * .......................#...#......................
 * .......................#...#......................
 * ......................#.....#.....................
 * ......................#.....#.....................
 * .....................#.......#....................
 * .....................#.......#....................
 * ....................#.........#...................
 * ....................#.........#...................
 * ..................................................
 * ..................................................
 * ..................................................
 */
