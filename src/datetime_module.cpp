...
namespace DateTime
{
    ...
    static void handleWifiMessages(void *subscriber_ptr, lv_msg_t *msg)
    {
        ...
        if (msg_id == static_cast<uint32_t>(MessageIDs::WiFiMessagesEnum::WIFI_GOT_IP_MSG))
        {
            ...
            static TaskHandle_t datetimeTaskHandle = NULL; // Static handle to avoid re-creation
            if (datetimeTaskHandle == NULL)
            {
                xTaskCreate(datetimeSyncTask, "datetimeSync", 10 * 1024, NULL, 11, &datetimeTaskHandle); // Slightly higher priority than weather/coin
            }
        }
    }
    ...
    void datetimeSyncTask(void *parameter)
    {
        ...
        // set notification call-back function - Moved to datetimeSyncTask, datetime module's responsibility
    ...
    // Callback function (get's called when time adjusts via NTP) - Moved to datetime_module.cpp
