...
// ... (Static variables and other function implementations from
// button_module.cpp) ...

void setupButton(LilyGo_Class &amoled_instance,
                 Adafruit_NeoPixel *
                     pixels_ptr) // Modified setupButton to take amoled instance
{
  amoled_ptr =
      &amoled_instance; // Store the amoled instance in the module's scope
  pixels = pixels_ptr;
  const BoardsConfigure_t *boards = amoled_ptr->getBoardsConfigure();
  // Set button on/off charge led , just for test button ,only 1.47 inch amoled
  // available
  if (boards->buttonNum) {
    ...
    for (int i = 0; i < boards->buttonNum; ++i) {
      ...
      buttonConfig->setEventHandler(
          buttonHandleEvent); // buttonHandleEvent is in Button namespace now
    }
  ...
  // Home Button Callback - Keep in button module for now, might move to UI
  // module later
  amoled_ptr->setHomeButtonCallback(
      [](void *ptr) {
...
void buttonHandleEvent(AceButton *button, uint8_t eventType,
                       uint8_t buttonState) {
  uint8_t id;
  ...
  const BoardsConfigure_t *boards =
      amoled_ptr
          ->getBoardsConfigure(); // You might need to get amoled object passed
                                  // to this function or make it accessible
  id = amoled_ptr->getBoardID();  // Same here, amoled object needed

  switch (eventType) {
  case AceButton::kEventClicked:
    if (button->getId() == 0) {
      // selectNextItem(); // UI function - we'll deal with this later (UI
      // module)
    } else {
      if (boards->pmu && id == LILYGO_AMOLED_147) {
        // Toggle CHG led
        amoled_ptr->setChargingLedMode(amoled_ptr->getChargingLedMode() !=
                                               XPOWERS_CHG_LED_OFF
                                           ? XPOWERS_CHG_LED_OFF
                                           : XPOWERS_CHG_LED_ON);
      }
  ...
  case AceButton::kEventLongPressed:
    ...
    // vTaskDelete(vUpdateDateTimeTaskHandler); } if (vUpdateCoin360TaskHandler)
    // { vTaskDelete(vUpdateCoin360TaskHandler); } if
    // (vUpdateWeatherTaskHandler) { vTaskDelete(vUpdateWeatherTaskHandler); }
...

    {
      ...
      amoled_ptr->sleep(touchpad_sleep); // Keep display sleep for now - might
                                         // move to power module later
    }
    ...
    if (boards->pmu && id == LILYGO_AMOLED_147) {
      ...
      // Set BOOT button as wakeup source - Keep for now, might move to power
      // module
