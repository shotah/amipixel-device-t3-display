...
{
    // --- Static variables ---
    WeatherApiData weatherApi; // Definition of the global weatherApi instance (within the namespace now)
    ...
    static void handleWifiMessages(void *subscriber_ptr, lv_msg_t *msg)
    {
        ...
        if (msg_id == static_cast<uint32_t>(MessageIDs::WiFiMessagesEnum::WIFI_GOT_IP_MSG))
        {
            ...
            static TaskHandle_t weatherTaskHandle = NULL; // Static handle to avoid re-creation
            if (weatherTaskHandle == NULL)
            {
                xTaskCreate(weatherUpdateTask, "weatherUpdate", 10 * 1024, NULL, 10, &weatherTaskHandle); // Example priority, adjust as needed
            }
        }
    }

    static void weatherUpdateTask(void *ptr) // Made static and void (no ptr parameter needed for now)
    {
        int httpCode = -1;
        ...
        url += std::to_string(WeatherLocation::LATITUDE); // Access LATITUDE from constants.h
        url += "&lon=";
        url += std::to_string(WeatherLocation::LONGITUDE); // Access LONGITUDE from constants.h
        ...
        client.setCACert(OpenWeatherRootCA); // Assuming OpenWeatherRootCA is defined in root_ca.h

        if (https.begin(client, url.c_str())) // Use client and https, and url.c_str() for std::string
        {
            httpCode = https.GET();
            if (httpCode > 0)
            {
                ...
                {
                    std::string httpBody = https.getString().c_str(); // Get string as std::string

                    // --- JSON Parsing ---
                    root = cJSON_Parse(httpBody.c_str());
                    if (root != nullptr) // Check for null after parsing
                    {
                        // ... (JSON parsing logic - mostly the same, but using weatherApi.city = ... and weatherApi.description = ... )
                        cJSON *cityJson = cJSON_GetObjectItem(root, "name");
                        if (cityJson != nullptr)
                        {
                            weatherApi.city = cityJson->valuestring; // Assign to weatherApi.city (std::string)
                        }
                        ...
                        if (weather != nullptr)
                        {
                            ...
                            {
                                cJSON *descriptionJson = cJSON_GetObjectItem(weatherItem, "description");
                                if (descriptionJson != nullptr)
                                {
                                    weatherApi.description = descriptionJson->valuestring; // Assign to weatherApi.description (std::string)
                                }
                            }
                        }
                        ...
                        Serial.print("\tCity:");
                        Serial.println(weatherApi.city.c_str()); // Use c_str() to print std::string
                        Serial.print("\tdescription:");
                        Serial.println(weatherApi.description.c_str()); // Use c_str() to print std::string
                        ...
                        lv_msg_send(MessageIDs::WEATHER_MSG_ID, &weatherApi); // Send weather data via lv_msg
