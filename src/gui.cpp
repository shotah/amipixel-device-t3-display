...
static void update_date(lv_timer_t *e) {
  ...
  lv_label_set_text_fmt(time_label, "%02d%s%02d", timeinfo.tm_hour,
                        colon != 0 ? "#ffffff :#" : "#000000 :#",
                        timeinfo.tm_min);
  colon = !colon;
...
void createBrightnessUI(lv_obj_t *parent) {
  ...
  // lv_obj_add_event_cb( label, [](lv_event_t *e) {
  //     lv_obj_t *label = (lv_obj_t *)lv_event_get_target(e);
  //     lv_label_set_text_fmt(label, "Temperature:%.2f°C",
  //     amoled.readCoreTemp());
  // }, LV_EVENT_MSG_RECEIVED, NULL);
...
void createPixelsUI(lv_obj_t *parent) {
  ...
  lv_obj_set_size(cont_row, lv_disp_get_physical_hor_res(NULL),
                  lv_disp_get_ver_res(NULL) - 10);
  lv_obj_align(cont_row, LV_ALIGN_TOP_MID, 0, 5);
  lv_obj_set_flex_flow(cont_row, LV_FLEX_FLOW_ROW);

  lv_obj_t *cont_col = lv_obj_create(parent);
  lv_obj_set_size(cont_col, lv_disp_get_physical_hor_res(NULL),
                  lv_disp_get_ver_res(NULL) - 10);
  ...
  lv_obj_set_style_text_color(slider_label, lv_color_white(), LV_PART_MAIN);
  lv_obj_add_event_cb(slider, pixels_event_handler, LV_EVENT_VALUE_CHANGED,
                      slider_label);
  lv_obj_align_to(slider_label, slider, LV_ALIGN_CENTER, 0, 0);
...
void weather_event_cb(lv_event_t *e) {
  ...
  case 0: // City
    ...
    break;
  case 1: // Max Min Temp
    lv_label_set_text_fmt(label, "Min/Max:%.1f/%.1f°C", api->temp_min,
                          api->temp_max);
    break;
  case 2: // Humidity
...
void createWeatherUI(lv_obj_t *parent) {
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    lv_obj_set_size(obj, lv_disp_get_physical_hor_res(NULL) / 3 - 15,
                    lv_disp_get_ver_res(NULL));
...
void createWiFiConfigUI(lv_obj_t *parent) {
  ...
  const char *android_url =
      "https://github.com/EspressifApp/EsptouchForAndroid/releases/download/"
      "v2.3.2/esptouch-v2.3.2.apk";
  const char *ios_url =
...
void factoryGUI(Adafruit_NeoPixel *pixels_ptr) {
  ...
  lv_obj_set_size(tileview, lv_disp_get_hor_res(NULL),
                  lv_disp_get_ver_res(NULL));
  lv_obj_add_event_cb(tileview, tileview_change_cb, LV_EVENT_VALUE_CHANGED,
                      NULL);
...
void createPaintUI(lv_obj_t *parent) {
  ...
  lv_obj_add_event_cb(ui.brushColorButton, onBrushColorLabelEvent,
                      LV_EVENT_CLICKED, NULL);
  lv_obj_add_event_cb(ui.brushButton, onBrushLabelEvent, LV_EVENT_CLICKED,
                      NULL);
  lv_obj_add_event_cb(ui.brushWidthSlider, onBrushWidthSliderEvent,
                      LV_EVENT_ALL, NULL);
  lv_obj_add_event_cb(ui.brushColorwheel, onBrushColorwheelEvent, LV_EVENT_ALL,
                      NULL);
  lv_obj_add_event_cb(ui.refreshButton, onRefreshLabelEvent, LV_EVENT_CLICKED,
                      NULL);
  lv_obj_add_event_cb(ui.canvas, onCanvasEvent, LV_EVENT_ALL, NULL);
}